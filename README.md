...
| `cx-slopslap` | [`vibedesignlab/slopslap`](https://github.com/vibedesignlab/slopslap) — MIT; optional [`Gesso-Build/skills`](https://github.com/Gesso-Build/skills) check-only runtime — MIT | Preserves the slopslap taxonomy, data, references, and scripts while adapting them to Codex. Optionally runs pinned Gesso `check --json` only for self-contained rendered HTML; its rules and auto-fix path are not bundled. |
