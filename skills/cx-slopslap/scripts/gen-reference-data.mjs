 ...
 * 각 값에 실제 출처(패키지명·버전)를 박아 referenceData.js 를 쓴다. 재생성 = `bash scripts/run-node-deps.sh gen-references`.
 *
 * dependency는 pinned `npx` isolated runner가 npm cache에 준비한다. 무키·무료(MIT).
 ...
 * 생성: \`bash scripts/run-node-deps.sh gen-references\`
...
fs.writeFileSync(path.join(ROOT, 'assets/data/referenceData.js'), body);
