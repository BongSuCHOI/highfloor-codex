...
# ///
# --- How to run ---
# uv run --isolated --with pytest==9.1.1 pytest scripts/tests/test_cli_contract.py -v
...
def test_find_searches_all_platforms_and_explains_matches(tmp_path: Path) -> None:
    ...
    for item in results:
        ...
        finder = shlex.quote(str(SKILL_ROOT / "scripts" / "find-agent-sessions.py"))
        assert item["detail_hint"] == f"uv run --isolated python {finder} read {item['id']} --platform {item['platform']}"
...
def test_read_summarizes_first_and_last_user_prompts(tmp_path: Path) -> None:
    ...
    finder = shlex.quote(str(SKILL_ROOT / "scripts" / "find-agent-sessions.py"))
    assert result["detail_hint"] == f"uv run --isolated python {finder} read claude-beta --platform claude"
