...
def _detail_hint(item: Session) -> str:
    finder = Path(__file__).resolve().parents[1] / "find-agent-sessions.py"
    return (
        f"uv run --isolated python {shlex.quote(str(finder))} "
        f"read {shlex.quote(item.id)} --platform {shlex.quote(item.platform)}"
    )
