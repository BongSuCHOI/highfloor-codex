...
describe("buildFreshnessBanner", () => {
  ...
  it("uses singular grammar for a graph one project commit behind", () => {
    ...
    expect(banner).toEqual({
      ...
      action:
        "Run $cx-understand-codebase analyze to refresh it before relying on impact or onboarding answers.",
      changedFiles: ["src/auth.ts"],
    });
  });
  ...
  it("explains an ahead domain graph with the correct refresh command", () => {
    ...
    expect(banner).toMatchObject({
      ...
      action:
        "Run $cx-understand-codebase domain to refresh it before relying on impact or onboarding answers.",
    });
  });
