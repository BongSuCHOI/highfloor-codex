...
function readSourceFile(url: URL) {
  ...
  if (!graphFile) {
    return rejectFileRequest("No knowledge graph found. Run $cx-understand-codebase analyze first.", 404);
  }
      ...
      configureServer(server) {
        ...
        server.middlewares.use((req, res, next) => {
          ...
          for (const candidate of candidates) {
            ...
            // Nodes can contain filePath values like /workspace/company/src/auth.ts.
          ...
          if (pathname === "/knowledge-graph.json") {
            res.setHeader("Content-Type", "application/json");
            res.end(JSON.stringify({ error: "No knowledge graph found. Run $cx-understand-codebase analyze first." }));
          } else {
            res.end();
          }
        });
