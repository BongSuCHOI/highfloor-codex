...
if (!graphDir) {
  console.error(
    ...
    "Generate one with $cx-understand-codebase analyze first, or pass the project directory as an argument.",
  );
  process.exit(1);
}
...
function serveGraphJson(res, fileName) {
  ...
  if (fileName === "knowledge-graph.json") {
    sendJson(res, 404, { error: "No knowledge graph found. Run $cx-understand-codebase analyze first." });
  } else {
    res.statusCode = 404;
    res.end();
  }
...
async function readGraphFreshness() {
  ...
  if (!fs.existsSync(knowledgeGraph)) {
    return {
      statusCode: 404,
      payload: { error: "No knowledge graph found. Run $cx-understand-codebase analyze first." },
    };
  }
