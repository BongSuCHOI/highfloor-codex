...
Before writing the script, create its input JSON file. Use the absolute
`$UA_DIR` supplied by the orchestration contract; do not re-resolve it. Write
the JSON to `$UA_DIR/tmp/ua-arch-input.json` with the host's file-editing
capability rather than interpolating project data through a shell:
...
```bash
node "$UA_DIR/tmp/ua-arch-analyze.js" \
  "$UA_DIR/tmp/ua-arch-input.json" \
  "$UA_DIR/tmp/ua-arch-results.json"
```
