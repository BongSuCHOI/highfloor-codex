...
Use the absolute `$UA_DIR` supplied by the orchestration contract. Do not
re-resolve or redirect it inside this worker:
...
- excludes persistent `.ua/` and legacy `.understand-anything/` analysis data so
  prior outputs never become source evidence for the next run
...
Write the input JSON to `$UA_DIR/tmp/ua-import-map-input.json` with the host's
file-editing capability. The `files[]` array is exactly Step B's `files[]`;
pass it through verbatim and do not interpolate target content through a shell:
