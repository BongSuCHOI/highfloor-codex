...
the JSON to `$UA_DIR/tmp/ua-tour-input.json` with the host's file-editing
...
```bash
node "$UA_DIR/tmp/ua-tour-analyze.js" \
  "$UA_DIR/tmp/ua-tour-input.json" \
  "$UA_DIR/tmp/ua-tour-results.json"
```
