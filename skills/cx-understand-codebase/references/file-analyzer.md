...
Create the input file with the batch data. **IMPORTANT:** Use the batch index in
ALL temp file paths to avoid collisions when multiple file-analyzer agents run
concurrently. Use the absolute `$UA_DIR` supplied by the orchestration contract;
do not re-resolve or redirect it inside this worker.
...
`$UA_DIR/tmp/ua-file-analyzer-input-<batchIndex>.json` with the host's
file-editing capability. Do not interpolate target content through a shell.
...
```bash
node "<UPSTREAM_SKILL_DIR>/extract-structure.mjs" \
...
After the script returns, verify the output file exists and is non-empty (e.g.
`test -s "$UA_DIR/tmp/ua-file-extract-results-<batchIndex>.json"`). Exit 0 with
