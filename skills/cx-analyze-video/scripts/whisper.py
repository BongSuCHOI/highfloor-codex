...
def load_api_key(preferred: str | None = None) -> tuple[str, str] | tuple[None, None]:
    ...
    # Never inspect a target repository's .env for credentials. Only an
    # explicit process environment or the dedicated watch config may supply a
    # key for external transcription.
...
def _post_whisper(endpoint: str, api_key: str, model: str, audio_path: Path) -> dict:
    ...
    headers = {
        ...
        "User-Agent": "highfloor-cx-analyze-video/1.0 (python-urllib)",
    }
...
def transcribe_video(
    ...
    *,
    ...
    if not upload_authorized:
        raise SystemExit(
            "External Whisper upload is disabled. Use --allow-whisper-upload only "
            "after explicit user authorization for this video's privacy and cost boundary."
        )
    ...
    if not backend or not api_key:
        ...
        raise SystemExit(
            ...
            "in the environment or in ~/.config/highfloor/cx-analyze-video.env. "
            f"Run `python3 {setup_py} --init-config` to create a dedicated template."
        )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            ...
            "[--backend groq|openai] --allow-whisper-upload",
            file=sys.stderr,
        )
        raise SystemExit(2)
    ...
    segments, backend = transcribe_video(
        ...
        upload_authorized="--allow-whisper-upload" in sys.argv,
    )
