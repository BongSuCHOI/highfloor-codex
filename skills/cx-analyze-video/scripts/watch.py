#!/usr/bin/env python3
"""Video-analysis entry point: download video, extract frames, parse transcript.

Prints a Markdown report to stdout listing frame paths and transcript. The
calling Codex host then inspects each frame path with its image-viewing tool.
"""
...
def main() -> int:
    ...
    ap.add_argument(
        "--subtitle-langs",
        default="en.*",
        help="yt-dlp subtitle selector in preference order (default: en.*). "
             "Example: ko.*,en.*",
    )
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="Allow audio upload to the selected Whisper API when captions are missing. "
             "Use only after explicit user authorization for external transfer and cost.",
    )
    ap.add_argument(
        "--whisper",
        choices=["groq", "openai"],
    ...
    auto_work_dir = args.out_dir is None
    if args.out_dir:
    ...
    if auto_work_dir:
        (work / ".highfloor-cx-analyze-video-workdir").write_text(
            "created-by=cx-analyze-video\n",
            encoding="utf-8",
        )
    ...
    if detail == "transcript" and not cue_timestamps and (
        transcript_segments or not args.allow_whisper_upload or args.no_whisper
    ):
        video_path = None
    else:
        if url_source:
    ...
    if (
        ...
        if backend and api_key:
            try:
                all_segments, used_backend = transcribe_video(
                    ...
                    backend=backend,
                    api_key=api_key,
                    upload_authorized=args.allow_whisper_upload,
                )
                transcript_segments = filter_range(all_segments, start_sec, end_sec) if focused else all_segments
                transcript_text = format_transcript(transcript_segments)
        ...
        and meta.get("has_audio")
        and not args.no_whisper
        and not args.allow_whisper_upload
    ):
        print(
            "[watch] captions unavailable; external Whisper upload is disabled by default. "
            "Re-run with --allow-whisper-upload only after explicit user authorization.",
            file=sys.stderr,
        )
    ...
    if detail != "transcript":
        ...
        if frame_meta.get("fallback"):
            fallback_candidates = frame_meta.get("fallback_candidate_count", 0)
            fallback_from = frame_meta.get("fallback_from", "primary")
            fallback = f" fallback after {fallback_candidates} {fallback_from} candidate"
            if fallback_candidates != 1:
                fallback += "s"
        deduped = frame_meta.get("deduped_count", 0)
        dedup_note = f", {deduped} near-duplicate{'s' if deduped != 1 else ''} dropped" if deduped else ""
        candidate_count = frame_meta.get("candidate_count", detail_count)
        candidate_label = "candidate" if candidate_count == 1 else "candidates"
        print(
            f"- **Frames:** {detail_count} selected from {candidate_count} {candidate_label} "
            f"({engine}{fallback}{dedup_note}, {range_mode} range, budget {target}, cap {cap_label})"
        )
    elif not cue_frames:
    ...
    if frames:
        ...
        print()
        print(
            "**Inspect each frame path below with the host image-viewing tool.** "
    ...
    if transcript_text:
        ...
        print(
            ...
            "Captions were missing and external Whisper transcription was unavailable "
            "or not authorized. After explicit authorization, configure a dedicated key "
            f"with `python3 {setup_py} --init-config` and re-run with "
            "`--allow-whisper-upload`._"
        )

    print()
    print("---")
    cleanup_py = SCRIPT_DIR / "cleanup.py"
    if auto_work_dir:
        print(f"_Work dir: `{work}` — clean with `python3 {cleanup_py} {work}` when done._")
    else:
        print(f"_Work dir: `{work}` — caller-managed because `--out-dir` was supplied._")
