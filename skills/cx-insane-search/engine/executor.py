...
path (browser automation must be driven from the Codex session itself).
"""
...
def _chrome_channel_available() -> bool:
    """Local templates require Node plus npx; packages resolve in npm's cache."""
    return _node_available() and shutil.which("npx") is not None


def _npx_package_args() -> list[str]:
    manifest = os.path.join(TEMPLATES_DIR, "package.json")
    with open(manifest, encoding="utf-8") as handle:
        dependencies = json.load(handle).get("dependencies", {})
    return [f"--package={name}@{version}" for name, version in dependencies.items()]
...
def _run_node_template(template: str, args: dict, timeout: int = 90) -> tuple[int, str, str]:
    ...
    try:
        shell_command = (
            ...
            f"node {shlex.quote(path)}"
        )
        env = os.environ.copy()
        env["PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD"] = "1"
        proc = subprocess.run(
            ["npx", "--yes", *_npx_package_args(), "-c", shell_command],
...
def run_playwright_fallback(
    ...
    *,
    ...
    if choice.startswith("playwright_mcp"):
        att.error = (
            "Browser automation must be invoked from the Codex session — "
            "use the available browser tools directly instead of fetch_chain."
        )
    ...
    if not _chrome_channel_available():
        att.error = (
            "node/npx not available for local Playwright template; "
            "install Node.js only after user approval"
        )
        att.verdict = Verdict.UNKNOWN.value
        att.elapsed_s = round(time.time() - t0, 3)
        return att, ""
