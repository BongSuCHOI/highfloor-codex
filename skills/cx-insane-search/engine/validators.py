...
# Above this size a single soft marker is evidence of a content mention, not a
# challenge by itself.
...
def _marker_pattern(marker: str) -> re.Pattern:
    # A token embedded at the tail of a longer identifier (for example,
    # "octocaptcha") is not the marker. Structural markers may legitimately
    # prefix longer values, so only the left boundary is constrained.
    return re.compile(r"(?<![a-z0-9_])" + re.escape(marker.lower()))


_HARD_PATTERNS = [(marker, _marker_pattern(marker)) for marker in HARD_CHALLENGE_MARKERS]
_SOFT_PATTERNS = [(marker, _marker_pattern(marker)) for marker in SOFT_CHALLENGE_MARKERS]


def _hard_marker_hits(body_lower: str) -> list[str]:
    return [marker for marker, pattern in _HARD_PATTERNS if pattern.search(body_lower)]


def _soft_marker_hits(body_lower: str) -> list[str]:
    return [marker for marker, pattern in _SOFT_PATTERNS if pattern.search(body_lower)]
