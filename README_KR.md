...
<p align="center">
  ...
  하나의 사고 경로를 강요하지 않으면서 최소 신뢰성을 높입니다.
</p>
...
> Highfloor의 목표는 하나의 사고 경로를 강요하지 않으면서 최소 신뢰성을
> 높이는 것입니다. 권한, 증거, 결과의 심각성, 관측 가능성 또는 실행 상태
> 때문에 중요한 결정이 아직 열려 있을 때만 절차를 추가합니다.
...
중요한 결정·주장·경계가 권한이나 증거만으로 아직 해결되지 않았거나,
결과의 심각성·되돌리기 어려움·관측 가능성·실행 상태 때문에 바로 행동하기
위험할 때 Highfloor는 그 간극을 닫을 수 있는 가장 작고 유용한 절차를
제공합니다.

모델이 스스로 확신이 낮다고 느낀다는 이유만으로 절차를 추가하지 않습니다.
해당 절차가 책임지는 질문이 신뢰할 수 있는 증거나 동등하거나 더 강한
검증으로 이미 닫혔다면 절차를 압축하거나 생략할 수 있습니다. 질문, 조사,
계획, 위임, 검증 또는 산출물을 더하기 전에는 중요한 결정을 바꾸거나 중요한
오류를 막을 가능성이 있고 그 가치가 비용을 정당화하는 가장 작은 행동을
우선합니다.

### Open Ceiling

Open Ceiling은 강한 모델에게 주는 특권이 아니라 증거로 얻는 재량입니다.
Hard Floor가 보존된 상태에서 어떤 단계가 책임지는 질문이 권위 있는 증거나
동등하거나 더 강한 검증으로 이미 해결되었다면 그 단계를 압축, 대체 또는
생략할 수 있습니다.

그 자유의 경계는 모델 이름이나 스스로 충분히 강하다는 주장보다 관측 가능한
결과와 증거가 정합니다. Highfloor는 모든 모델에 같은 사고 과정을 강요하지
않고 결과와 경계를 안내합니다.

### 증거에 따라 진화합니다

스킬, 에이전트, 지침과 워크플로는 영구적인 성취가 아니라 검증해야 할
engineering hypothesis입니다. 신뢰성 향상이 routing, context, latency,
coordination과 유지 비용을 넘는다는 비교 증거가 있는 동안에만 유지합니다.
더 작은 시스템으로도 같은 바닥 또는 더 나은 바닥을 유지할 수 있다면
`NO_CHANGE`, 흡수, 병합과 삭제도 성공적인 결과입니다. 공통 평가 계약은
[`docs/EVALUATION.md`](docs/EVALUATION.md)를 따릅니다.
...
신뢰할 수 있는 자동화에는 신뢰 가능한 도메인 역량에 접근할 수 있어야 하고
결과를 판별할 현실적인 검증 수단이 있어야 합니다. 그 역량은 직접 경험한
실무자뿐 아니라 권위 있는 표준, 검증된 데이터, 외부 전문가 검토와 그 밖의
신뢰할 수 있는 증거에서도 올 수 있습니다. 도메인 역량과 현실적인 검증
수단이 모두 없다면 자동화는 권위 있는 결론이 아니라 탐색적 결과로 남아야
합니다.
...
- 지표는 목표 그 자체가 아니라 목표를 판단하는 증거로 취급
- 중요한 결정을 바꾸거나 중요한 오류를 막을 가능성이 있고 비용을 정당화할
  때만 추가 증거, 절차 또는 위임을 사용
...
| `cx-slopslap` | [`vibedesignlab/slopslap`](https://github.com/vibedesignlab/slopslap) — MIT; 선택적 [`Gesso-Build/skills`](https://github.com/Gesso-Build/skills) check-only runtime — MIT | slopslap taxonomy, data, reference와 script를 보존해 Codex에 맞게 수정했습니다. Self-contained rendered HTML에만 pinned Gesso `check --json`을 선택적으로 실행하며, Gesso rule과 auto-fix path는 번들하지 않습니다. |
